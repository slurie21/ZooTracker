...
namespace ZooTracker.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Logout()
        {
            ...
            await _unitOfWork.EnterpriseLogging.Add(new EnterpriseLogging { App = "ZooTracker", Area = "Auth", Note = $"Logging out {userID}", CreatedDate = DateTime.UtcNow, CorrelationID = correlationID ?? Guid.NewGuid().ToString() });
            await _unitOfWork.Save();
        ...
        public async Task<IActionResult> RefreshToken([FromBody] string refreshToken)
        {
            ...
            await _unitOfWork.EnterpriseLogging.Add(new EnterpriseLogging { App = "ZooTracker", Area = "Auth", Note = $"Removed old refresh token for {user.UserName} and added new one", CreatedDate = DateTime.UtcNow, CorrelationID = correlationID ?? Guid.NewGuid().ToString() });
            await _unitOfWork.Save();
